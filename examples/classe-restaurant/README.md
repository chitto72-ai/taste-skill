...
- **i18n** (inlined dictionary + `tr()` engine): a `tr()` lookup keyed by the Italian source string, with graceful fallback to Italian for anything untranslated. `BlurText` and the page components resolve their text through `tr()`, so the menu, headings, copy and labels all localize. Dish proper names (e.g. *Spaghetto al Gambero Viola*, *King Crab*, *Wagyu*) and the "(ENG)" PDF labels stay in their original language by design. The dictionary is embedded directly in `index.html` so the language selector works even when the file is opened on its own (no separate JS file to keep alongside it).
